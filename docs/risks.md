...
| R-15 | 模拟器 ArkWeb（HarmonyOS 6.1.1 镜像）执行 CM bundle（0.20 与 6.x、minify 与 unminify 均实测）导致应用进程崩溃；613KB 同尺寸 padding 与 tiny 稳定——**任何真实 CM 代码内容均触发进程级崩溃**，JS 错误捕获无法拦截 | 确认 | 中 | 模拟器仅验证 ArkWeb 基础能力（rawfile/桥/返回值）；CM 编辑矩阵唯一路径=真机（R-13 解锁后），真机为 OpenHarmony 7.0 ArkWeb（代际更新，需实测） | 真机验证后 |
