...
> 计划：SDD 原风险 | 实测风险。最后更新：2026-08-31（M0-03 真机矩阵完成）。
...
| R-18 | API 26 Beta SDK 无 @kit.ShareKit：FR-SHARE 系统分享面板不可用 | 已发生 | 中 | V0.1 以剪贴板通道分享原始 Markdown（分享前先落盘）；正式 SDK 复核后接入 ShareKit | 正式 SDK 发布后 |
